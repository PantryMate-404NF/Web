...
`check:design-tokens`는 핵심 폰트·색상·테마·반응형 토큰의 존재를 검사합니다. `check:token-usage`는 실제 UI 역할에 `bg-white`, `text-white`, 직접 브랜드색·일반 텍스트색을 다시 쓰지 않는지 검사하며, 금지된 arbitrary color utility는 뒤따르는 공백·따옴표 등 class 구분자와 관계없이 검출합니다.
