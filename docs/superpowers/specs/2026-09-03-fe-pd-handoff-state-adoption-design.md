...
FE·PD 핸드오프 문서에서 확정된 상태 구분을 현재 팬트리 화면과 공용 UI에 반영한다. 아직 디자인 확정이 필요한 Toast, Retry CTA, Empty 일러스트는 구현하지 않는다.
...
- 팬트리 Error 상태는 향후 API 재시도 함수를 주입할 수 있도록 `onRetry` 인터페이스만 제공한다. 버튼의 문구·배치가 확정되기 전에는 Retry CTA를 임의로 렌더링하지 않는다.
