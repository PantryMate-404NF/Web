...
| `npm run check:design-tokens` | 핵심 Pretendard·색상·라이트/다크·반응형 토큰 존재 확인 |
| `npm run check`               | 디자인 토큰·포맷·린트·타입 검사                        |
| `npm run test`                | Vitest 실행                                            |
| `npm run build`               | 디자인 토큰 검사 후 프로덕션 빌드                      |
...
- 라이트·다크 테마의 실제 대비와 상태별 시각적 일관성
- 화면 코드에서 primitive 색상값을 직접 사용하지 않았는지 여부
- 접근성의 실제 사용성
...
디자인 토큰의 소유권과 사용 규칙은 [디자인 시스템](../design/design-system.md)을 따른다.
