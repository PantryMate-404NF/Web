...
describe('Next API proxy', () => {
  ...
  it('브라우저의 /api 요청을 원격 Gateway의 같은 API 경로로 전달한다', async () => {
    ...
    vi.stubEnv('PANTRY_RECIPE_API_BASE_URL', 'http://localhost:8082');
    ...
    await expect(config.rewrites?.()).resolves.not.toContainEqual({
      destination: 'http://localhost:8082/api/recipes/:path*',
