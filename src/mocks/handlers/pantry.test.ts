...
describe('pantryHandlers', () => {
  it('기본 목록 요청에 공통 성공 응답을 반환한다', async () => {
    ...
    const payload = (await response.json()) as ApiResponse<PantryItemDto[]>;

    expect(response.status).toBe(200);
    expect(payload.status).toBe('SUCCESS');
    expect(payload.data).toEqual([
      expect.objectContaining({ pantryItemId: 1, ingredientName: '대파' }),
    ]);
  });

  it('empty mock은 성공 응답과 빈 배열을 반환한다', async () => {
    const response = await fetch('http://localhost:8080/api/pantry-items?mock=empty');
    const payload = (await response.json()) as ApiResponse<PantryItemDto[]>;
  ...
  it('unauthorized mock은 인증 오류 응답을 반환한다', async () => {
    const response = await fetch('http://localhost:8080/api/pantry-items?mock=unauthorized');
    const payload = (await response.json()) as ApiResponse<PantryItemDto[]>;
  ...
  it('error mock은 서버 오류 응답을 반환한다', async () => {
    const response = await fetch('http://localhost:8080/api/pantry-items?mock=error');
    const payload = (await response.json()) as ApiResponse<PantryItemDto[]>;
